...
export default class Photo extends Component {
  ...
  _renderImage = () => {
    ...
    let zoomSize = {
      width: this.state.progress < 1 ? 0 : screen.width,
      height: this.state.progress < 1 ? 0 : screen.height,
    }
    if (this.props.zoom) {
      return (
        <ImageZoom cropWidth={zoomSize.width}
          onClick={() => { this.props.toggleControls() }}
          onDoubleClick={() => { this.props.toggleControls() }}
          onLongPress={() => { this.props.toggleControls() }}
          leaveStayTime={300}
          cropHeight={zoomSize.height}
          imageWidth={sizeStyle.width}
          imageHeight={sizeStyle.height}>
          <Image
            {...this.props}
